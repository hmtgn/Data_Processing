...
object Main {
  def main(args: Array[String]) {
    ...
    val climateRelatedNewsCount = spark.sql("SELECT COUNT(*) FROM news WHERE containsWordGlobalWarming = true").first().getLong(0)
    val nonClimateRelatedNewsCount = spark.sql("SELECT COUNT(*) FROM news WHERE containsWordGlobalWarming = false").first().getLong(0)
