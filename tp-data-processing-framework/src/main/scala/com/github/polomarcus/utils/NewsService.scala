...
object NewsService {
  ...
  def enrichNewsWithClimateMetadata(newsDataset: Dataset[News]) : Dataset[News] = {
    newsDataset.map { news =>
      val containsClimateRelatedWords = ClimateService.isClimateRelated(news.title) || ClimateService.isClimateRelated(news.description)
      ...
      val enrichedNews = News(
        ...
        news.url,
        news.urlTvNews,
        containsClimateRelatedWords, // Applying ClimateService.isClimateRelated function
        news.media
      )
